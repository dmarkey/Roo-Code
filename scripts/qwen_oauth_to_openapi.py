...
def is_token_valid(credentials: Dict[str, Any]) -> bool:
    ...
    # Check if expiry_date is a valid number
    try:
        expiry_date = int(credentials['expiry_date'])
...
def refresh_access_token(credentials: Dict[str, Any], custom_path: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if not response.ok:
        try:
            ...
            if 'error' in error_data:
                raise ValueError(f"Token refresh failed: {error_data['error']} - {error_data.get('error_description', '')}")
            else:
                raise ValueError(f"Token refresh failed: {response.status_code} {response.reason}. Response: {error_text}")
        except ValueError:
            # Re-raise if it's already a ValueError
            raise
        except Exception:
            # Handle case where response is not valid JSON
            raise ValueError(f"Token refresh failed: {response.status_code} {response.reason}. Response: {response.text}")
    
    # Parse the response
    try:
        token_data = response.json()
    except Exception as e:
        raise ValueError(f"Failed to parse token refresh response as JSON: {e}")
    ...
    try:
        save_qwen_credentials(new_credentials, custom_path)
    except Exception as e:
        print(f"Warning: Failed to save refreshed credentials to file: {e}")
...
def get_openapi_credentials(custom_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not is_token_valid(credentials):
        try:
            credentials = refresh_access_token(credentials, custom_path)
        except Exception as e:
            # Log the error but continue with existing credentials
            # They might still work if they're not completely expired
...
def main(custom_path: Optional[str] = None):
    """Main function to demonstrate usage."""
    try:
        # Get OpenAPI credentials
        openapi_creds = get_openapi_credentials(custom_path)
        ...
        if 'expiry_date' in credentials:
            ...
            try:
                ...
                print(f"Token Expiry Date: Invalid timestamp ({credentials['expiry_date']})")
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert Qwen OAuth credentials to OpenAPI format")
